...
export function ExpertClustersSection() {
  ...
  useEffect(() => {
    ...
    `,
      {},
      { next: { revalidate: 3600 } } // caching aman untuk deploy
    )
      .then(setData)
          ...
          {clusters.map((item: any, i: number) => {
            ...
            return (
              <motion.div
                ...
                className={`
                  relative group p-10 rounded-[2.5rem] min-h-[360px] flex flex-col items-center text-center
                  bg-gradient-to-br ${colorTheme}
                  shadow-[inset_0_-4px_8px_rgba(0,0,0,0.05),0_15px_30px_-10px_rgba(0,0,0,0.15)]
                  transition-all duration-500
                  ${hasExperts ? "cursor-pointer hover:-translate-y-4 hover:scale-[1.03] hover:shadow-xl" : ""}
                `}
              >
